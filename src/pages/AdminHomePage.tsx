    ...
    <div className="min-h-screen bg-stage-950 text-ink-900">
      <header className="sticky top-0 z-20 border-b border-stage-700 bg-stage-950/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4 md:px-8">
          ...
          <button
            onClick={onCreate}
            className="rounded-2xl bg-amber px-4 py-3 text-sm font-extrabold text-stage-950 shadow-tile transition hover:-translate-y-0.5 active:translate-y-1 active:shadow-tile-active"
          >
            + Tạo Game
          </button>
        </div>
      ...
      <main className="mx-auto max-w-6xl px-5 py-8 md:px-8 md:py-10">
        <section className="mb-8 grid gap-4 md:grid-cols-[1.6fr_1fr]">
          <div className="rounded-[28px] border border-stage-700 bg-gradient-to-br from-stage-800 to-stage-900 p-6 md:p-8">
            ...
            <p className="mt-4 max-w-2xl text-sm leading-6 text-ink-500 md:text-base">
              Thiết kế bộ câu hỏi, chọn visual và sau đó đưa Game lên màn hình
              lớn cho người chơi tham gia bằng QR hoặc Game PIN.
            </p>
            <button
              onClick={onCreate}
              className="mt-7 rounded-2xl bg-white px-5 py-3 font-display text-sm font-extrabold text-stage-950 transition hover:scale-[1.02]"
            >
              Bắt đầu tạo Game →
            </button>
          </div>
          <div className="rounded-[28px] border border-stage-700 bg-stage-800 p-6">
            ...
            <div className="mt-5 space-y-4">
              {[
                ["01", "Game PIN", "Một mã cho toàn bộ Game"],
                ["02", "Multi-question", "Nhiều câu hỏi trong cùng một lobby"],
                ["03", "Presentation", "Sẵn sàng cho màn hình TV / projector"],
              ].map(([n, title, desc]) => (
                <div key={n} className="flex gap-4">
                  <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-white/8 text-xs font-extrabold text-amber">
                    {n}
                  </span>
                  <div>
        ...
        <div className="mb-4 flex items-end justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-ink-500">
              Quản lý các bộ câu hỏi và phiên live.
            </p>
          </div>
          <button
            onClick={() => void load()}
            className="text-xs font-bold text-ink-900/45 hover:text-ink-900"
          >
            ↻ Làm mới
          </button>
        </div>
        ...
        {error && (
          <div className="mb-4 rounded-2xl border border-coral/30 bg-coral/10 p-4 text-sm text-coral">
            {error}
          </div>
        )}
        {games === null && (
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {[1, 2, 3].map((x) => (
              <div
                key={x}
                className="h-48 animate-pulse rounded-[24px] bg-white/5"
              />
            ))}
          </div>
        )}
        {games?.length === 0 && (
          <div className="rounded-[28px] border border-dashed border-stage-700 bg-white/[0.02] px-6 py-16 text-center">
            <div className="mx-auto grid h-16 w-16 place-items-center rounded-2xl bg-amber/15 text-3xl">
              +
            </div>
            ...
            <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-ink-500">
              Tạo Game đầu tiên và thêm nhiều câu hỏi vào cùng một bộ.
            </p>
            <button
              onClick={onCreate}
              className="mt-6 rounded-2xl bg-amber px-5 py-3 text-sm font-extrabold text-stage-950"
            >
              Tạo Game đầu tiên
            </button>
          </div>
          ...
          {games?.map((game) => {
            ...
            return (
              <div
                ...
                className="group cursor-pointer overflow-hidden rounded-[24px] border border-stage-700 bg-stage-800 text-left transition hover:-translate-y-1 hover:border-stage-700 hover:shadow-2xl"
              >
                <div className="relative h-32 overflow-hidden bg-gradient-to-br from-stage-700 to-stage-900">
                  ...
                  <span
                    className={`absolute left-4 top-4 rounded-full px-3 py-1 text-[11px] font-extrabold ${status.className}`}
                  >
                    {status.label}
                  </span>
                  <span className="absolute bottom-3 right-4 rounded-lg bg-black/30 px-2.5 py-1 font-mono text-xs font-bold tracking-[0.18em] text-ink-900">
                    {game.pin}
                  </span>
                </div>
                <div className="flex items-start justify-between gap-3 p-5">
                  <div className="min-w-0">
                    <h3 className="line-clamp-2 font-display text-lg font-extrabold">
                      {game.title}
                    </h3>
