  ...
  return <div className="min-h-screen bg-stage-950 text-ink-900">
    <header className="sticky top-0 z-30 border-b border-stage-700 bg-stage-950/90 backdrop-blur-xl">
      <div className="mx-auto flex max-w-[1400px] items-center justify-between gap-4 px-4 py-3 md:px-6">
        <button onClick={onBack} className="rounded-xl px-3 py-2 text-sm font-bold text-ink-500 hover:bg-white/5 hover:text-ink-900">← My Games</button>
    ...
    <main className="mx-auto grid max-w-[1400px] gap-5 p-4 md:grid-cols-[240px_minmax(0,1fr)_320px] md:p-6">
      <aside className="rounded-[24px] border border-stage-700 bg-stage-900 p-3">
        <div className="flex items-center justify-between px-2 py-2"><span className="text-xs font-extrabold uppercase tracking-[0.15em] text-ink-500">Questions</span><button onClick={addQuestion} className="grid h-8 w-8 place-items-center rounded-xl bg-white/8 font-bold hover:bg-white/15">+</button></div>
        <div className="mt-2 space-y-2">
          {questions.map((q, i) => <div key={q.id} className={`group flex items-center gap-2 rounded-2xl p-2 transition ${i === activeQuestion ? "bg-white/10 ring-1 ring-white/15" : "hover:bg-white/5"}`}>
            <button onClick={() => setActiveQuestion(i)} className="flex min-w-0 flex-1 items-center gap-3 text-left"><span className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl font-display text-xs font-extrabold ${i === activeQuestion ? "bg-amber text-stage-950" : "bg-white/8 text-ink-500"}`}>{i + 1}</span><span className="truncate text-xs font-semibold text-ink-700">{q.question || "Câu hỏi mới"}</span></button>
            {questions.length > 1 && <button onClick={() => removeQuestion(i)} className="px-1 text-xs text-ink-900/20 hover:text-coral">✕</button>}
          </div>)}
        </div>
        <button onClick={addQuestion} className="mt-3 w-full rounded-xl border border-dashed border-stage-700 py-3 text-xs font-bold text-ink-900/45 hover:border-stage-700 hover:text-ink-900">+ Thêm câu hỏi</button>
      </aside>
      ...
      <form onSubmit={submit} className="space-y-5">
        ...
        <section className="rounded-[28px] border border-stage-700 bg-stage-900 p-5 md:p-7">
          <div className="mb-6 flex items-start justify-between gap-4"><div><p className="text-xs font-extrabold uppercase tracking-[0.18em] text-amber">02 · QUESTION {activeQuestion + 1}</p><h2 className="mt-2 font-display text-2xl font-extrabold">Nội dung câu hỏi</h2></div><span className="rounded-full bg-white/8 px-3 py-1.5 text-xs font-bold text-ink-900/45">{current.durationSeconds}s</span></div>
        ...
        <section className="rounded-[28px] border border-stage-700 bg-stage-900 p-5 md:p-7">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {current.options.map((option, i) => <div key={i} className="flex items-center gap-2"><span className={`grid h-11 w-11 shrink-0 place-items-center rounded-xl text-sm font-extrabold ${["bg-coral","bg-sky","bg-emerald","bg-amber"][i % 4]} text-stage-950`}>{String.fromCharCode(65 + i)}</span><input value={option} onChange={(e) => updateOption(i, e.target.value)} placeholder={`Đáp án ${String.fromCharCode(65 + i)}`} className="field-input" />{current.options.length > 2 && <button type="button" onClick={() => removeOption(i)} className="px-1 text-ink-300 hover:text-coral">✕</button>}</div>)}
          </div>
          {current.options.length < 8 && <button type="button" onClick={addOption} className="mt-4 rounded-xl border border-stage-700 px-4 py-2.5 text-xs font-bold text-ink-500 hover:bg-white/5 hover:text-ink-900">+ Thêm lựa chọn</button>}
        </section>
      ...
      <aside className="md:sticky md:top-[82px] md:h-fit">
        <div className="overflow-hidden rounded-[28px] border border-stage-700 bg-stage-900">
          ...
          <div className="p-3"><div className="relative aspect-[4/3] overflow-hidden rounded-[22px] p-5" style={previewStyle}>
            ...
            <div className="relative flex h-full flex-col justify-between"><div className="flex justify-between"><span className="rounded-lg bg-black/20 px-2 py-1 text-[10px] font-bold text-ink-700">Q{activeQuestion + 1}</span><span className="rounded-lg bg-black/20 px-2 py-1 font-mono text-[10px] font-bold text-ink-700">{current.durationSeconds}s</span></div><p className="font-display text-lg font-extrabold leading-6 text-ink-900 drop-shadow-md">{current.question || "Câu hỏi của bạn sẽ hiện ở đây"}</p><div className="grid grid-cols-2 gap-2">{current.options.slice(0,4).map((o,i) => <div key={i} className="rounded-xl bg-white p-2.5 text-[10px] font-extrabold text-stage-950 shadow-lg">{o || `Đáp án ${String.fromCharCode(65+i)}`}</div>)}</div></div>
          </div></div>
          <div className="grid grid-cols-2 gap-3 px-4 pb-4"><div className="rounded-xl bg-white/5 p-3"><p className="text-[10px] text-ink-300">Câu hỏi</p><p className="mt-1 font-display text-lg font-extrabold">{questions.length}</p></div><div className="rounded-xl bg-white/5 p-3"><p className="text-[10px] text-ink-300">Game PIN</p><p className="mt-1 font-mono text-lg font-extrabold tracking-wider text-amber">AUTO</p></div></div>
        </div>
        <p className="mt-3 px-2 text-[11px] leading-5 text-ink-300">Game PIN sẽ được sinh tự động sau khi tạo. Một PIN dùng cho toàn bộ các câu hỏi.</p>
      </aside>
    </main>
  </div>;
}
